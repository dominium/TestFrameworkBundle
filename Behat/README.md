...
**@Labudzinski\TestFrameworkBundle\Behat\ServiceContainer\OroTestFrameworkExtension** provides integration with Oro BAP based applications.
  ...
  **Do**:
  ...
  ```yaml
      Workflow Edit Form:
        ...
        class: Labudzinski\TestFrameworkBundle\Behat\Element\Form
        options:
          mapping:
...
use Labudzinski\TestFrameworkBundle\Behat\Context\OroFeatureContext;
...
```Labudzinski\TestFrameworkBundle\Behat\ServiceContainer\OroTestFrameworkExtension``` is used for building testing suites.
...
default: &default
  suites:
    AcmeDemoBundle:
      ...
      contexts:
        - Labudzinski\TestFrameworkBundle\Tests\Behat\Context\OroMainContext
...
Every isolator must implement ```Labudzinski\TestFrameworkBundle\Behat\Isolation\IsolatorInterface``` and ```oro_behat.isolator``` tag with priority.
...
3. the ```class``` namespace for element's class (should be extended from ```Labudzinski\TestFrameworkBundle\Behat\Element\Element```).
When omitted, the ```Labudzinski\TestFrameworkBundle\Behat\Element\Element``` class is used by default.
...
use Labudzinski\TestFrameworkBundle\Behat\Element\Element;
...
Every Page element should extend from ```Labudzinski\TestFrameworkBundle\Behat\Element\Page```.
...
use Labudzinski\TestFrameworkBundle\Behat\Element\Page;
